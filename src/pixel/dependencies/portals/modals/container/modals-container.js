...
export default class ModalsContainer extends WebComponent {
  ...
  componentDidMount() {
    ...
    modalsService
      ...
      .subscribe(this.handlePushModal.bind(this));

    modalsService
      .getPopModalObservable()
      .pipe(this.takeUntilLifeCycle())
      .subscribe(this.handlePopModal.bind(this));
  }
  ...
  ensureCloseModalViaBackdrop() {
    ...
    if (
      this.contentEl.lastChild &&
      !this.contentEl.lastChild.isActionRequired
    ) {
      this.closeModalViaBackdropSubscription =
        this.enableCloseModalViaBackdrop();
    }
  }

...
export default class MapComponentHelperModal extends ModalAdapter {
  ...
  componentDidMount() {
    this.listRenderController = new ListRenderControllerBuilder(this.containerListEl)
      ...
      .withOnAfterBindItem((itemEl, item) => {
        ...
        item.name === 'Nome' && itemEl.setAttribute('input.value', this.payload.name);
    ...
    Rxjs.fromEvent(this.editBtnEl, 'click')
      .pipe(this.takeUntilLifeCycle())
      .subscribe(() => {
        const placeholderList = Array.from(this.containerListEl.children).map((element) => {
          const key = this.listRenderController.renderAdapter.extractKeyFromEl(element);
          ...
          return {
            ...
            inputValue: item.name === 'Sublinhado' ? element.checked : element.querySelector('input').value,
          };
        });
  ...
  #handleInputValue(itemEl, item, layer) {
    ...
    if (item.name === 'Grossura') {
      ...
      itemEl.setAttribute('input.value', this.#convertToRgbToHex(stroke.getColor()));
    } else if (item.name === 'Cor interna') {
      itemEl.setAttribute('input.value', this.#convertToRgbToHex(fill.getColor()));
    } else if (item.name === 'Sublinhado') {
      itemEl.checked = stroke.getLineDash();
    } else {
      itemEl.setAttribute('input.value', ol.color.asArray(fill.getColor()).slice()[3]);
    }
  }
  ...
  #convertToRgbToHex(rgba) {
    ...
    return `#${this.#colorToHex(convertedRgba[0])}${this.#colorToHex(convertedRgba[1])}${this.#colorToHex(
      convertedRgba[2]
    )}`;
  }
